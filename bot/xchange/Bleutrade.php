...
class Bleutrade extends Exchange {
  ...
  public function getTickers( $currency ) {
    ...
    foreach ( $markets as $market ) {
      ...
      $split = explode( "_", $market[ 'MarketName' ] );
      if ( $split[ 1 ] != $currency ) {
        continue;
      }
      $ticker[ $split[ 0 ] ] = $market[ 'Last' ];
    }
  ...
  public function refreshExchangeData() {
    ...
    foreach ( $currencies as $data ) {
      ...
      if ( $coin == 'BTC' || array_search( $coin, $tradeables ) !== false ) {
  ...
  private function queryOrderbook( $tradeable, $currency ) {
    return $this->xtractResponse( $this->queryPublicJSON( Bleutrade::PUBLIC_URL . 'getorderbook?depth=1&type=ALL&market=' . $tradeable . '_' . $currency ) );
  }
  private function queryMarkets() {
    return $this->xtractResponse( $this->queryPublicJSON( Bleutrade::PUBLIC_URL . 'getmarkets' ) );
  }
  private function queryCurrencies() {
    return $this->xtractResponse( $this->queryPublicJSON( Bleutrade::PUBLIC_URL . 'getcurrencies' ) );
  }
  private function queryMarketSummary() {
    return $this->xtractResponse( $this->queryPublicJSON( Bleutrade::PUBLIC_URL . 'getmarketsummaries' ) );
  }
  private function queryCancelOrder( $ordid ) {
  ...
  private function queryAPI( $method, $req = [ ] ) {
    ...
    $uri = 'https://bleutrade.com/api/v2/' . $method . '?' . http_build_query( $req );
